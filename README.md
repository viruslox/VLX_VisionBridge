...
- **Input Type**: `local` (folder of media), `srt`, `rtmp` (and `rtmps`), `webrtc`, `rtsp` (and `rtsps`), `overlay`. For `local`, folder combinations are automatically parsed (video only, image + audio, image only, audio only).
...
Defines the base canvas size for FFmpeg filtergraphs.
...
### Layers (`layers`)
Defines up to 10 layers to compose the scene.
...
- **`input_type`**: The type of input source. Options:
  ...
  - `overlay`: Captures HTML web pages using a natively spawned headless Chromium process via `xvfb-run` and FFmpeg's `x11grab`.
