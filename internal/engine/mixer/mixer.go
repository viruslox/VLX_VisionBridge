...
func BuildFilterComplex(cfg *models.Config) ([]string, string, string, string) {
	...
	args = append(args, "videotestsrc", "pattern=black", "is-live=true", "!", "video/x-raw,width=1920,height=1080,framerate=30/1", "!", "comp.sink_0")
	args = append(args, "compositor", "name=comp", "!")
	args = append(args, "queue", "leaky=downstream", "max-size-buffers=1", "max-size-time=30000000", "!", "videoconvert", "!", "video/x-raw,format=RGBA", "!", "videoconvert", "!")
	...
	if cfg.Input.ChromiumSource.Active {
		...
		for i, layer := range cfg.Input.MediaSource.Layers {
			if layer.Active {
				...
				args = append(args, "src_"+fmt.Sprint(i)+".", "!", "queue", "leaky=downstream", "max-size-buffers=1", "max-size-time=30000000", "!", "videoconvert", "!", fmt.Sprintf("comp.sink_%d", i+1))
				args = append(args, "src_"+fmt.Sprint(i)+".", "!", "queue", "leaky=downstream", "max-size-buffers=1", "max-size-time=30000000", "!", "audioconvert", "!", "audioresample", "!", fmt.Sprintf("acomp.sink_%d", i+1))
			}
		}
	}
	return args, "", "", ""
