...
func TestBuildFilterComplex(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedFilterStr: "color=s=1920x1080:c=black [base];\n[0:v] scale=1920:-1 [v0_scaled];\n[base][v0_scaled] overlay@layer1=x=0:y=0 [out0];\n[0:a] aresample=48000:async=1,aformat=sample_rates=48000:channel_layouts=stereo,asetpts=N/SR, volume@layer1=1.00 [a0];\n[out0] zmq=b=tcp\\\\://127.0.0.1\\\\:5555 [v_out];\n",
			expectedVideoPad:  "[v_out]",
			expectedAudioPad:  "[a0]",
		},
		{
		...
		},
		{
			...
			expectedFilterStr: "color=s=1920x1080:c=black [base];\n[0:a] aresample=48000:async=1,aformat=sample_rates=48000:channel_layouts=stereo,asetpts=N/SR, volume@layer1=0.50 [a0];\n[base] zmq=b=tcp\\\\://127.0.0.1\\\\:5555 [v_out];\n",
			expectedVideoPad:  "[v_out]",
			expectedAudioPad:  "[a0]",
		},
		{
