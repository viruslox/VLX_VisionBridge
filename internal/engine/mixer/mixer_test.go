...
func TestBuildFilterComplex(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedArgs:      nil,
			expectedFilterStr: "color=s=1920x1080:r=30:c=black [base];\nanullsrc=r=48000:cl=stereo [a_out];\n[base] zmq=b=tcp\\\\://127.0.0.1\\\\:5555 [v_out];\n",
		...
		},
		{
			...
			expectedFilterStr: "color=s=1920x1080:r=30:c=black [base];\n[0:v] setpts=PTS-STARTPTS,scale=1920:-1 [v0_scaled];\n[base][v0_scaled] overlay@layer1=x=0:y=0 [out0];\n[0:a] aresample=48000:async=1,aformat=sample_rates=48000:channel_layouts=stereo,asetpts=PTS-STARTPTS, volume@layer1=1.00 [a0];\n[out0] zmq=b=tcp\\\\://127.0.0.1\\\\:5555 [v_out];\n",
			expectedVideoPad:  "[v_out]",
			expectedAudioPad:  "[a0]",
		},
		{
		...
		},
		{
			...
			expectedFilterStr: "color=s=1920x1080:r=30:c=black [base];\n[0:a] aresample=48000:async=1,aformat=sample_rates=48000:channel_layouts=stereo,asetpts=PTS-STARTPTS, volume@layer1=0.50 [a0];\n[base] zmq=b=tcp\\\\://127.0.0.1\\\\:5555 [v_out];\n",
			expectedVideoPad:  "[v_out]",
			expectedAudioPad:  "[a0]",
		},
		{
			...
			expectedFilterStr: "color=s=1920x1080:r=30:c=black [base];\n[0:v] setpts=PTS-STARTPTS,scale=1280:-1 [v0_scaled];\n[base][v0_scaled] overlay@layer1=x=0:y=0 [out0];\n[0:a] aresample=48000:async=1,aformat=sample_rates=48000:channel_layouts=stereo,asetpts=PTS-STARTPTS, volume@layer1=1.00 [a0];\n[1:v] setpts=PTS-STARTPTS,scale=640:-1 [v1_scaled];\n[out0][v1_scaled] overlay@layer2=x=1280:y=0 [out1];\n[1:a] aresample=48000:async=1,aformat=sample_rates=48000:channel_layouts=stereo,asetpts=PTS-STARTPTS, volume@layer2=0.50 [a1];\n[a0][a1] amix=inputs=2:duration=longest:dropout_transition=0 [a_out];\n[out1] zmq=b=tcp\\\\://127.0.0.1\\\\:5555 [v_out];\n",
			expectedVideoPad:  "[v_out]",
			expectedAudioPad:  "[a_out]",
		},
		{
			...
			expectedArgs:      []string{"-f", "x11grab", "-thread_queue_size", "128", "-framerate", "30", "-video_size", "1920x1080", "-draw_mouse", "0", "-use_wallclock_as_timestamps", "1", "-i", ":99"},
			expectedFilterStr: "anullsrc=r=48000:cl=stereo [a_out];\n[0:v] zmq=b=tcp\\\\://127.0.0.1\\\\:5555 [v_out];\n",
			expectedVideoPad:  "[v_out]",
			expectedAudioPad:  "[a_out]",
		},
		{
