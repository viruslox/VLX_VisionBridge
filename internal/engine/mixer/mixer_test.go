...
func TestBuildFilterComplex(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedArgs:      []string{"-f", "x11grab", "-thread_queue_size", "1024", "-video_size", "1920x1080", "-draw_mouse", "0", "-i", ":99"},
			expectedFilterStr: "color=s=1920x1080:r=30:c=black [base];\n[0:v] setpts=PTS-STARTPTS,colorkey=0x00FF00:0.1:0.1 [chroma_chromium];\n[base][chroma_chromium] overlay=x=0:y=0 [out_chromium];\nanullsrc=r=48000:cl=stereo [a_out];\n[out_chromium] zmq=b=tcp\\\\://127.0.0.1\\\\:5555 [v_out];\n",
			expectedVideoPad:  "[v_out]",
			expectedAudioPad:  "[a_out]",
		},
		{
